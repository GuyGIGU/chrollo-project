...
| 2026-08-23 | **Chain specimens**: rule the keep/junk sheet — 20 candidates per chain mined from the cache; 2–3 KEEPs per chain opens the next stage (deep two-structure marks come later, when the marking tool lands). Naming your own charts instead/in addition works too. | The wall fixture (plan Task 3) + every threshold the program will propose | **[story_chain_candidates_2026-08.md](story_chain_candidates_2026-08.md)** — the sheet |
...
| 2026-08-22 | **Redeploy + log rotation**: `update_dashboard.bat` (the retirement + fixes go live), and the elevated `nssm set ChrolloDashboard AppRotate…` block. **Amended 2026-08-24 — the flood's CAUSE is now fixed in code**, so rotation drops from load-bearing to backstop: `/ibkr/status` was missing from the request middleware's quiet list and wrote 99.1% of the 167 MB error log; it is now quieted and the streams are split (INFO→stdout, WARNING+→stderr). The redeploy is what makes that real — until then the log keeps growing. Still owed by you: the elevated `AppRotate` run, and a ruling on deleting the current 167 MB file. | Live dashboard + disk | decisions.md 2026-08-24 row |
