...
Like the trend model and the L2 reader, the layer is **measure-only** — it moves no rail, gates nothing, scores nothing — and nothing on the live scan path computes it yet: fire-path staging, behind its own flag with the full flag protocol, is a later Event Map stage (PLAN-event-tape.md Task 6). The narrative-role layer (spring / test / SOS / LPS roles *over* these mechanical swings, fed the elected bricks) is the stage after this one.
