...
| `tools/run_maturation.bat` | `tools/ops/run_maturation.bat` | The Windows scheduled task "Chrollo Forward Returns" was registered with this absolute path and still runs it (read 2026-09-25). It only calls `tools/ops/` and passes back the exit code; `tests/tooling/test_maturation_launcher.py` pins it. Remove it once the task points at `tools/ops/` (one elevated command, the operator's: `docs/deploy.md` §4b), with the test's forwarder check. |
