...
verbatim; `rejection` → **touch_and_pivot** (SIGNED — his phrase); S-family
`test`/`failed` → **support_test** with the outcome on the verdict axis;
unresolved waves/episodes → **support_test / resistance_test** with verdict
**open** (typing before resolution would be lookahead by naming); episode
outcomes → verdicts (completed→held, failed→breached, open→open,
unreadable→unreadable).

Signed the same day: the breach verdict is **breached**, not "gave" — his
official term for price crossing a boundary, ONE word for both rails; what a
breach means is context's job, never the verdict's. He also ruled the three
