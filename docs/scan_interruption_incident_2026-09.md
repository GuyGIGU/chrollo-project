...
| status `aborted` | Re-run the scan | It was stopped before it finished. **Two populations since 2026-09-07:** rows written when closing the page terminated the child (council review finding 4 — a manual job now runs on its own thread and the page re-attaches to it), and rows the operator's own **Stop** button writes (review A3, the replacement lever for a wedged child). |
