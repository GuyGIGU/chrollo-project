...
- [ ] **`AR_FIRST_REACTION_ENABLED` flip** — **ROW SUPERSEDED TWICE; corrected 2026-08-19.** It used to read "needs an A/B re-run + flip/kill call (kill-by 2026-08-15)"; both are discharged. **RULED DO-NOT-FLIP 2026-08-14** (flag-OFF closer to the operator's own marks on the cohort chosen to flatter the proposal), kill-by now **2026-09-15**, and the blocking decision moved off the eyeball onto the climax anchor. **The anchor was then mechanised 2026-08-19** — see [`climax_anchor_diagnosis_2026-08-19.md`](climax_anchor_diagnosis_2026-08-19.md): the drawn climax is the extreme of a fixed 60-trading-day window ending at the box open, its polarity keyed on `root.kind` (a keying already **Tested-DEAD** 2026-07-27). **The 15–20 operator marks this item used to owe are WITHDRAWN** — the diagnosis needed none. What remains is one operator ruling: re-key the polarity off the covering segment (behaviour change, needs its own A/B), or accept the anchor as a diagnostic that feeds nothing and let the flag expire on 2026-09-15 with that document as the written reason. *(The `engine/ar-first-reaction` branch holds only an older raw-bar prototype superseded on `main` — do not resurrect; this is a decision, not a build.)*
