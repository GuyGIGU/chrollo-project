...
- [ ] **EGBN high-shelf / BUEC LPS detector** — EGBN misses because its LPS closes ride *above* R (a high-tight-flag shelf) which `find_lps` rejects as `terminal_low` (~85% of the miss). Distinct above-R form, same family as the live holding-shelf LPS. Optional span-aware occupancy sub-measure enters report-only (measure-first). **Do NOT touch the box-election respect gate to chase EGBN.**
