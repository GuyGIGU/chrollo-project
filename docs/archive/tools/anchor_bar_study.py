...
Anchor-bar study â€” what makes a bar THE operator's rail anchor.
...
level with k resting neighbors. This instrument studies the choice directly â€”
all 33 Guided List marks carry his declared r_anchor_date / s_anchor_date and
which rail he drew first â€” and characterizes, per side:

  * fidelity   â€” is the drawn rail the anchor bar's own High/Low? (the
                 rails-at-H/L ruling, measured)
  * rank       â€” where the anchor bar's extreme ranks among the drawn
                 window's bars (1 = the window extreme)
  * ignored    â€” how much outlier the drawn rail leaves outside (window
                 extreme minus rail, in ATRs)
  * position   â€” where the anchor bar sits in the drawn window (0..1)
  * support    â€” bars whose extreme rests within 0.25 ATR of the rail
  * character  â€” the anchor bar's spread vs the window median, volume vs
                 Vol_50, close position inside its own range, local-pivot
                 flag (extreme vs +-3 neighbors)
  * proven     â€” touches of the rail AFTER the anchor bar (TOUCH_TOLERANCE_ATR)
  * engine     â€” the engine's own anchor bars for the elected (hits) or
    ...
    for s in setups:
        ...
        if mark is None or not mark.r_anchor_date or not mark.s_anchor_date:
            raise SystemExit(f"anchor study: {setup_key(s)} lacks declared "
                             "anchor dates â€” refusing a partial population")
        rows.append(study_mark(s, mark, frames,
                               status_by_key.get(setup_key(s), "?")))

    print("=" * 78)
    print("  ANCHOR-BAR STUDY â€” the operator's declared rail anchors, measured")
    print("=" * 78)
