...
Bar-dwell fire A/B â€” the sealed campaign of docs/bar_dwell_protocol_2026-07.md.
...
Predicted (protocol Â§4): EGBN + YPF convert, each electing at drawn-box
...
# The drawn-box identity each MANDATORY conversion must elect at (protocol Â§4A:
...
def main() -> None:
    ...
    if base_hash != baseline["engine_config_version"]:
        raise SystemExit(
            ...
            f"current {base_hash[:16]} â€” reseal before running (evidence must pair)")

    print("=" * 78)
    print("  BAR-DWELL FIRE A/B â€” docs/bar_dwell_protocol_2026-07.md")
    print("=" * 78)
    print(f"baseline manifest: {base_hash[:16]}...   marks_fingerprint: "
          f"{baseline['marks_fingerprint'][:16]}...")
    base_junk = _junk_fires()
    if base_junk:
        raise SystemExit(f"baseline junk fires {base_junk} â€” corpus gate red; stop")
    ...
    verdict = "YES â€” flip per protocol Â§5" if (ok_a and ok_b and ok_e) \
        else "NO â€” record tested-DEAD per protocol Â§6"
