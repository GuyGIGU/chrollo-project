# Archive

Completed planning and coordination artifacts, kept for the decision paper trail.
These are **not** operating docs — the sources of truth stay live at the repo root
and under `docs/` (`strategy_v2.md`, `MAP.md`, `AGENTS.md`, `CLAUDE.md`, `deploy.md`,
`conventions.md`, `flag_ledger.md`).

Everything here describes work that already **shipped and merged** to `main`; each file
had zero live code/doc references at the time it was archived (2026-07-03 dead-code sweep):

- `PLAN-*.md` — Carmack-Council build plans whose features shipped; the durable spec for
  each lives on under `specs/` or in the code.
- `refactor_seed_recall_misses_codex.md`, `structure_reader_split_codex.md`,
  `structure_reader_upgrade_plan_for_claude.md`, `seed_recall_misses_findings.md` —
  a spent Codex↔Claude sprint-coordination cluster (2026-06-20); the LPS-variant work
  it tracked landed in `core/structure/`.

Git history retains full provenance; nothing here is load-bearing.
