...
- **`tools.audits.doctrine_audit` is a MANDATORY manual run-slot after any change touching engine
  ...
  `tests/tooling/test_doctrine_audit_plumbing.py` guards its plumbing on every default run, but only the
