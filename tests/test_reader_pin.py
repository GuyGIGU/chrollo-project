...
def test_reader_pin_reports_no_reading_drift():
    ...
    `check_baseline()` returns True iff every pinned reader surface on all 87
    charts (33 marks + 17 junk + 37 shadow, including the 5 rejecting tickers
...
def test_baseline_parses_and_covers_all_three_populations():
    ...
    # The junk corpus (17 since NVT left on the operator's Thu 10/09/2026
    # ruling, pinned Sun 13/09/2026) and the shadow fixture (37, including its
    # 5 rejecting tickers - the population every other guard is blind to) are
    # FIXED populations, so their counts stay pinned here.
    assert len(pops["junk"]) == 17
