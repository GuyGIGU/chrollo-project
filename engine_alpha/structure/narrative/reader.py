...
``engine_alpha.structure.narrative.bricks``. Those bricks are *injected* (``read_structure(...,
...
from engine_alpha.structure.lps.detection import detect_lps_tests, lps_range_threshold
...
def _walk_structure(df, atr, *, bricks=None, trace=None, near_miss=None,
    ...
    for i in range(_MAX_ANCHORS):
        ...
        # Phase A's climax polarity reads the trend floor; it computes it lazily
        # inside the guard, so only the reads that actually complete a story pay
        # for it rather than every anchor the walk visits.
