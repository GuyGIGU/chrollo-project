...
description: Execute a Carmack Council plan task by task. Use when explicitly asked to implement a plan, do a "council implement", "carmack implement", "council build", or invoke /council-implement. Reads the output of /council-plan and builds each task sequentially, loading the relevant expert's reference document per task. Verifies after each task (pytest + seed-recall/shadow + lint). Produces an implementation log for /council-review. Stack: Chrollo — Python / FastAPI / pandas quant engine / SQLite / React 19 + Vite / pytest.
---

# Carmack Council Implementer

> **Ultra Council (v2).** Part of the Ultra Council suite. Read `.council/council.config.md` (if present)
> for this project's detected gates + roster, and `conventions.md` for accepted patterns, before building
> — use the project's real gate commands for the per-task verification step (Phase 2, Step 4).
...
The Chrollo stack:
- **Python 3.11+** — the deterministic quant engine (`core/`) and the FastAPI backend.
...
- **pandas / numpy / scipy** — vectorised market-data processing. Lookahead bias and float/NaN correctness are first-class.
- **React 19 + Vite** — a plain SPA (no Next.js/RSC) with `lightweight-charts` + `recharts`. Styling follows `PRODUCT.md` + `DESIGN.md`.
- **yfinance (pinned) + IBKR live** — market data and broker integration.
- **pytest** + the project's seed-recall / shadow-harness / byte-parity regression patterns; byte-parity reproducibility is a hard requirement.
...
| Sebastián Ramírez (Backend) | `references/quality-backend.md` |
| Brandur Leach (Data Integrity) | `references/quality-postgres.md` |
| Wes McKinney (Numerical) | `references/quality-llm.md` |
| Pipeline Performance | `references/quality-backend.md` (performance sections) |
...
- **Apply the expert's principles.** The reference doc isn't background reading — it's the constraint set. If Ramírez's Principle 1 says crash on programmer errors and handle operational errors, your error handling must follow that model. If Brandur's principle says SQLite defaults (FK pragma, busy-timeout) need setting, set them. If McKinney's principle says never compare prices with `==`, use a tolerance.
...
```
# Run tests (includes the seed-recall and shadow-harness regression guards)
python -m pytest -q

# Lint / syntax sanity (use ruff if configured, else compileall)
ruff check . || python -m compileall core webapp tools

# Frontend lint (only if the task touched webapp/frontend)
cd webapp/frontend && npm run lint
```

If any verification step fails:
- **Test failures from your changes:** fix them before proceeding. A **seed-recall or shadow-baseline regression** is a hard stop — it means an engine change altered detection; do not proceed until it is understood and intended.
...
**Do NOT proceed to the next task if your changes break tests, linting, or — above all — byte-parity / seed-recall.** Fix first, then move on.
