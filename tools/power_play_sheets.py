...
The species census (`tools/power_play_census.py`) measures; THIS tool spends the
...
engine_config_version — a re-run on the same stamp redraws nothing.
    ...
    python -m tools.power_play_sheets --census output/power_play_census_species.json
    python -m tools.power_play_sheets ... --cache "..\\Chrollo Project\\market_data_cache_5y.parquet"
    python -m tools.power_play_sheets ... --no-render      # selection + SHEETS.md only
"""
...
def write_sheet(doc, sel, out_dir, census_path, wall_clock):
    ...
    md = [
        ...
        "",
        f"Generated by `python -m tools.power_play_sheets` from "
    ...
    md += [
        ...
        "",
        "```",
        "python -m tools.power_play_census --out output\\power_play_census_species.json",
        "python -m tools.power_play_sheets --census output\\power_play_census_species.json",
        "```",
        "",
    ]
