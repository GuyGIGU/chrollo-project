...
Usage:
    python -m tools.regression.shadow_diff --build-fixture   # one-time, after a real scan
    python -m tools.regression.shadow_diff --capture          # snapshot current outputs as baseline
    python -m tools.regression.shadow_diff --check            # fail (exit 1) on canonical-field drift
"""
...
try:  # works under both `python -m tools.regression.shadow_diff` and `python tools/regression/shadow_diff.py`
...
from core.calibration.replay import fired_window_walk
...
def _load_fixture() -> tuple[dict[str, pd.DataFrame], dict]:
    if not os.path.exists(_FIXTURE_PARQUET) or not os.path.exists(_FIXTURE_SCALARS):
        raise FileNotFoundError(
            "No fixture - run `python -m tools.regression.shadow_diff --build-fixture` first "
