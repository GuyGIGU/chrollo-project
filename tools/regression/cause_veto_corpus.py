...
it. The general negative corpus (``tools.regression.negative_corpus``) only proves a frame
...
Usage:
    python -m tools.regression.cause_veto_corpus --build-fixture   # one-time, from the live cache
    python -m tools.regression.cause_veto_corpus --check           # fail (exit 1) if the bite is gone
"""
...
try:  # works under both `python -m tools.regression.cause_veto_corpus` and `python tools/regression/cause_veto_corpus.py`
...
def _load_fixture() -> tuple[dict[str, pd.DataFrame], dict]:
    if not os.path.exists(_FIXTURE_PARQUET) or not os.path.exists(_FIXTURE_META):
        raise FileNotFoundError(
            "No cause-veto fixture - run `python -m tools.regression.cause_veto_corpus "
