...
The shadow guard (``tools.regression.shadow_diff``) deliberately compares only nine
...
itself is battery machinery and is committed. Hermetic: frozen fixture only.
    ...
    python -m tools.regression.fold_parity --capture <run-dir>/captures/pre.json
    python -m tools.regression.fold_parity --compare <run-dir>/captures/pre.json
    python -m tools.regression.fold_parity --compare pre.json --mapping rename_map.json
"""
...
try:  # works under both `python -m tools.regression.fold_parity` and `python tools/regression/fold_parity.py`
