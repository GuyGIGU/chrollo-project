...
Usage:
    python -m tools.maintenance.settings_reference --write   # regenerate the doc block in place
    python -m tools.maintenance.settings_reference --check   # fail (exit 1) if the doc drifted
"""
...
try:  # works under both `python -m tools.maintenance.settings_reference` and `python tools/maintenance/settings_reference.py`
...
def check_doc(doc_path: str = _DOC_PATH) -> bool:
    ...
    print("settings quick-reference block DRIFTED from config/settings.py - "
          "run `python -m tools.maintenance.settings_reference --write` and commit the doc.")
    return False
