...
Usage (ChrolloDashboard venv python, from repo root):
    python -m tools.research.shape_profile              # per-mark profiles + aggregate
    python -m tools.research.shape_profile --ticker DSGN
    python -m tools.research.shape_profile --json OUT   # per-mark JSON sidecar
"""
...
# --- bootstrap repo path (mirror tools/calibration/calibration_stat_card.py) -
try:  # works under both `python -m tools.research.shape_profile` and `python tools/research/shape_profile.py`
