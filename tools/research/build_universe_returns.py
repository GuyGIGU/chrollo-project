...
Run:
    python -m tools.research.build_universe_returns --out output/universe_returns.parquet

Then feed it to the harness:
    python -m tools.research.backtest_engine --universe output/universe_returns.parquet --json out.json
...
try:  # works under both `python -m tools.research.build_universe_returns` and `python tools/research/build_universe_returns.py`
