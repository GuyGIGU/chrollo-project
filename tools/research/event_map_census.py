...
Roles stay named: ``tools.calibration.calibration_stat_card`` measures drawn geometry,
``tools.calibration.shelf_harness`` grades the LPS detector, THIS instrument reads the
...
their content digests are verified by the ``tools.regression.marks_corpus --check``
...
Usage (ChrolloDashboard venv python, from repo root):
    python -m tools.research.event_map_census            # full report
    python -m tools.research.event_map_census --check    # pinned promotion regression
    python -m tools.research.event_map_census --json OUT # report rows as JSON
    python -m tools.research.event_map_census --ticker T # one mark's as-of drill-down
...
try:  # works under both `python -m tools.research.event_map_census` and `python tools/research/event_map_census.py`
