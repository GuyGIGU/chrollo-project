...
Usage (ChrolloDashboard venv python, from repo root):
    python -m tools.research.near_miss_census              # full report (walks engine)
    python -m tools.research.near_miss_census --json OUT   # walk once, cache rows
    python -m tools.research.near_miss_census --from IN    # re-render from cached rows
    python -m tools.research.near_miss_census --check      # pinned identity+headline gate
"""
...
try:  # works under both `python -m tools.research.near_miss_census` and `python tools/research/near_miss_census.py`
