...
(``read_structure`` in engine_alpha.structure.narrative.reader): it enumerates the candidate
...
calibrated bricks. It changes nothing and gates nothing.
    ...
    python -m tools.audits.structure_case_audit                       # default cases
    python -m tools.audits.structure_case_audit ROIV ADM BWMX
"""
...
try:  # works under both `python -m tools.audits.structure_case_audit` and `python tools/audits/structure_case_audit.py`
...
from engine_alpha.structure.box.box_primitives import _pivot_order
