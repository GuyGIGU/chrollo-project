...
Two populations, one state machine, two rails:
  ...
  drawn — every operator ``CalibrationMark(verdict='box')`` on HIS rails over
          HIS window (``tools.replay.drawn_box_window``, the shared EC-13
  ...
  junk  — every negative-corpus strict candidate framing that SURVIVED the
          ...
          respect), judged on ``tools.replay.judged_window``. 98 framings /
...
Usage (the repo venv, from the repo root):
    python -m tools.bar_state_census [--json OUT.json]
"""
...
try:  # works under both `python -m tools.bar_state_census` and direct
