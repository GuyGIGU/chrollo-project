...
Usage (ChrolloDashboard venv python, from repo root):
    python -m tools.calibration.calibration_stat_card            # print cards + aggregate
    python -m tools.calibration.calibration_stat_card --json OUT # also dump per-mark JSON
    python -m tools.calibration.calibration_stat_card --ticker EGBN
"""
...
# --- bootstrap repo path (mirror tools/calibration/calibration_harness.py) --
try:  # works under both `python -m tools.calibration.calibration_stat_card` and `python tools/calibration/calibration_stat_card.py`
