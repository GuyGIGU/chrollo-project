...
(``tools.calibration.replay.snapped_election``; changing snap semantics is a deliberate
re-freeze event, not a knob), and grade through the pure taxonomy
(``tools.calibration.agreement``). All rule variants run in ONE walk per mark — the
...
from tools.calibration import agreement, replay
