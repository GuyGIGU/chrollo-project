...
``tools.calibration.calibration_stat_card`` measures the DRAWN geometry descriptively;
...
Usage (ChrolloDashboard venv python, from repo root):
    python -m tools.calibration.shelf_harness [--ticker X] [--json OUT]
"""
...
try:  # works under both `python -m tools.calibration.shelf_harness` and `python tools/calibration/shelf_harness.py`
