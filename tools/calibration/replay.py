...
GATE (``tools.regression.marks_corpus``), the agreement harness
(``tools.calibration.calibration_harness``), the chronology battery
(``tools.audits.event_map_chronology``), the election/shelf instruments and tests,
...
try:  # works under both `python -m tools.calibration.replay` and `python tools/calibration/replay.py`
