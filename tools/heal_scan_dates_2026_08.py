...
local midnight — weekend re-scans of Friday's data filed as NEW
...
blanket assumption — so seed/manual cohorts whose stamps are true stay put.
...
What the evidence pass (2026-08-11) established, and how this tool acts:
  ...
  AMBIGUOUS               -> DEFERRED with --defer-ambiguous (the June
                             ...
                             different scan — then the shift MERGES: incumbent
...
def _non_trading_dates(cur) -> list[str]:
    ...
    Seed/manual rows are exempt from the whole heal — one exists on a
...
def _effective_verdict(cohort: dict, defer_ambiguous: bool) -> str:
    ...
    if v == "AMBIGUOUS":
        if defer_ambiguous:
            return "defer"
        sys.exit(f"AMBIGUOUS cohort {cohort['scan_date']} {cohort['universe_type']} — "
...
def _plan(cur, evidence: dict, defer_ambiguous: bool):
    ...
    staying/deferred cohort blocks the move — cascade-defer — unless the
    ...
    # Coverage is demanded only for screener-bearing cohorts — seed/manual
    ...
    for (d, u) in sorted(k for k, a in actions.items() if a == "prev"):
        ...
        if occupant is None:
            ...
            # slot (d, u) stays occupied — anything shifting onto it defers too
...
def _move(cur, d: str, u: str, t: str, merge: bool) -> tuple[int, int, int, int]:
    ...
    target key — a whole stamp-true cohort (the 2026-07-06 rescan) or a lone
    seed row sitting mid-range — blocks the mover, and the incumbent's row
    ...
    reviews = 0
    if cohort_tickers:  # setup_reviews has no universe column — bind by ticker
...
def main() -> None:
    ...
    if os.path.exists(BACKUP_PATH):
        sys.exit(f"Backup already exists — refusing to run twice: {BACKUP_PATH}")
    ...
    with con:  # ONE transaction — any failure rolls the whole heal back
        cur.execute(
    ...
    if not ok:
        sys.exit("POST-CHECKS FAILED — restore from the backup and investigate.")
