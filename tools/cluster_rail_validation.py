"""Cluster-rail statistic validation — all 33 drawn boxes (Rail Program Task 6).
...
docs/cluster_rail_validation_2026-07.md before any run — a definition that
matches only NKTR is a tail fit and the answer is NO (protocol §8).

Also reports the plain extreme anchor (max High / min Low of the drawn
window — the current candidate-anchor basis) so the delta the statistic buys
...
# Pre-registered acceptance (docs/cluster_rail_validation_2026-07.md §2) —
...
def main() -> None:
    ...
    print("=" * 78)
    print("  CLUSTER-RAIL VALIDATION — the resting-extreme statistic vs 33 drawn boxes")
    print("=" * 78)
