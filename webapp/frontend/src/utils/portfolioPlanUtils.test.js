...
const planFor = (position, trades) => (
  buildPortfolioPlanMap([position], trades).get(positionPlanKey(position))
);
...
test('buildPortfolioPlanMap links a live stock position to its open journal plan', () => {
  const plan = planFor(
    basePosition,
    [{
      ...baseTrade,
