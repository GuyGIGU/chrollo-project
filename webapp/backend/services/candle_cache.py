...
repo-root ``config`` package, so a module-level ``services.market_data`` /
``core.pipeline.downloads`` import would risk the boot-crash trap (AP-3).
"""
...
def _looks_rate_limited(symbol: str) -> bool:
    ...
    if rate_limit.in_cooldown():
        return True
    try:
        from core.pipeline.downloads import _is_yahoo_rate_limit_text  # noqa: PLC0415
    except Exception:
        return False
...
def _fetch(symbol: str, start_ts: pd.Timestamp, end_ts: pd.Timestamp,
    ...
    from services.market_data import daily_candle_frame  # noqa: PLC0415 — vendor chain, lazy
