...
Candles come from the ONE shared builder (core.pipeline.market_data.candles — the EC-3
...
the file open — core/pipeline/market_data/cache.py retries for a bounded ~5s):
...
def _clean_ticker_frame(df):
    ...
    from core.pipeline.market_data.candles import clean_daily_frame
...
def ticker_candles(ticker: str, universe_key: str | None = None) -> dict:
    ...
    """
    from core.pipeline.market_data.candles import chart_candles
...
def batch_candles(tickers: list[str], pins: dict | None = None) -> dict:
    ...
    retryable verdict.
    """
    from core.pipeline.market_data.candles import daily_candles
    from core.pipeline.json_safety import to_json_safe
