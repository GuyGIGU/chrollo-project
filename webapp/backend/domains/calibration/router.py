...
from domains.calibration.validation import TICKER_RE, parse_iso_date, validate_mark
...
def calibration_chart(ticker: str = Query(...), as_of: str = Query(...)):
    ...
    from core.pipeline.market_data.downloads import _price_regime, price_auto_adjust  # noqa: PLC0415 — lazy, yfinance-heavy chain
    from engine_alpha.freeze.manifest import manifest_hash  # noqa: PLC0415
    from domains.market_data.candle_cache import load_candles  # noqa: PLC0415 — session cache + resilient fetch
    from domains.market_data.service import chart_candles  # noqa: PLC0415
    ...
    from domains.calibration.frame_store import freeze_frame, freeze_grading_frame  # noqa: PLC0415 — file I/O module, lazy like the fetch chain
    try:
...
def calibration_engine_read(ticker: str = Query(...), as_of: str = Query(...),
    ...
    from domains.calibration.frame_store import load_frame  # noqa: PLC0415 — file I/O module, lazy
...
def calibration_agreement(ticker: str = Query(...), db: Session = Depends(get_db)):
    ...
    from domains.calibration.agreement import agreement_for_marks  # noqa: PLC0415 — harness/pandas chain, lazy
...
def calibration_frame_thumb(ticker: str = Query(...), as_of: str = Query(...),
    ...
    from domains.calibration.frame_store import load_frame, preview_series  # noqa: PLC0415 — file I/O module, lazy
    frozen = load_frame(symbol, as_of, digest=frame_digest)
    if frozen is None:
        _refuse(404, "unbound_frame",
...
def calibration_fired(ticker: str = Query(...), db: Session = Depends(get_db)):
    ...
    marks = (db.query(CalibrationMark)
             .filter(CalibrationMark.ticker == symbol)
             .order_by(CalibrationMark.as_of_date).all())
    from domains.calibration.fired import fired_for_marks  # noqa: PLC0415 — harness/pandas chain, lazy
...
def calibration_trigger_grade(ticker: str = Query(...), db: Session = Depends(get_db)):
    ...
    marks = (db.query(CalibrationMark)
             .filter(CalibrationMark.ticker == symbol)
             .order_by(CalibrationMark.as_of_date).all())
    from domains.calibration.trigger_grade import trigger_grade_for_marks  # noqa: PLC0415 — fired/harness chain, lazy
...
from domains.calibration.schemas import EventIn, MarkIn, EventOut, MarkOut
...
def _reject_unframed_trigger(payload: MarkIn):
    ...
    if payload.trigger_date is None:
        return
    from domains.calibration.frame_store import load_grading_frame  # noqa: PLC0415 — file I/O module, lazy
    ticker = (payload.ticker or "").strip().upper()
